...
class OverviewStrategy(DataCleaningStrategy):
    ...
    fund_create_time_pattern = re.compile(r'发行日期</th><td>(.*?)</td><th>成立日期')
    ...
    def fill_result(self, fund_response: FundResponse, context: FundContext) -> None:
        ...
        # 发行日期
        fund_create_time_result = self.fund_create_time_pattern.search(page_text)
        if fund_create_time_result:
            fund_create_time = fund_create_time_result.group(1)
            context.fund_create_time = NO_DATA if not fund_create_time else fund_create_time
